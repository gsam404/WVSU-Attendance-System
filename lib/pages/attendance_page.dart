...
import 'package:wvsu_attendance_system/pages/admin_session.dart'; // Admin ID check
import 'dart:async';
import 'dart:convert';
...
class _AttendancePageState extends State<AttendancePage> {
  ...
  Future<void> _fetchAttendance() async {
    ...
    if (_selectedFilterCategory == 'School Year' &&
        _selectedFilterValue != null) {
      params['school_year'] = _selectedFilterValue!;
    } else if (_selectedFilterCategory == 'Month' &&
        _selectedFilterValue != null) {
  ...
  Map<String, String> _calcMetrics(List<Map<String, dynamic>> data) {
    ...
    final topDept =
        deptCount.entries.reduce((a, b) => a.value >= b.value ? a : b).key;
    final topCourse =
        courseCount.entries.reduce((a, b) => a.value >= b.value ? a : b).key;
    final avg =
        (studentVisits.values.fold(0, (s, v) => s + v) / studentVisits.length)
            .toStringAsFixed(1);
    String peak = '—';
  ...
  Future<void> _exportCSV() async {
    ...
    try {
      ...
      await Printing.sharePdf(
        bytes: bytes,
        filename:
            'WVSU_Attendance_${DateFormat('yyyyMMdd').format(DateTime.now())}.csv',
      );
    } catch (e) {
      if (mounted) {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (ctx) => [
            pw.Text('Summary',
                style: pw.TextStyle(
                    font: fontBold, fontSize: 12, color: wvsuBlue)),
            pw.SizedBox(height: 6),
            pw.Table(
              border: pw.TableBorder.all(color: PdfColors.grey300, width: 0.5),
              children: [
                _pdfMetricRow('Total Entries', metrics['total']!, fontBold,
                    fontRegular, headerBg),
                _pdfMetricRow('Most Visited Department', metrics['dept']!,
                    fontBold, fontRegular, PdfColors.white),
                _pdfMetricRow('Most Visited Course', metrics['course']!,
                    fontBold, fontRegular, headerBg),
                _pdfMetricRow('Avg Visits / Student', metrics['avg']!, fontBold,
                    fontRegular, PdfColors.white),
                _pdfMetricRow('Peak Traffic Hour', metrics['peak']!, fontBold,
                    fontRegular, headerBg),
              ],
            ),
                ...
                children: [
                  ...
                  if (_isExporting)
                    const LinearProgressIndicator(
                        backgroundColor: Color(0xFFDDEAF8),
          ...
          : LayoutBuilder(
              builder: (ctx, constraints) => SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  ...
                  child: DataTable(
                    headingRowColor:
                        WidgetStateProperty.all(const Color(0xFFDDEAF8)),
                    dataTextStyle:
